...
plt1 <- ggplot(data = DFAlex, aes(x = weights, y = assims, colour = factor(year))) +
  ...
  labs(colour = "Year") +
  theme_minimal() +
  geom_function(fun = function(x) 0.5*x^(3/4)-0.3*x, linetype = "dashed") 
  #geom_function(fun = function(x) 0.1*x^(23/10)/(1+x^(23/10)), linetype = "dashed") 


ggsave(paste0("figures/weight_v_assim Alex ", scenario, " .png"), plot = plt1, width = 18, height = 20, unit = "cm")
