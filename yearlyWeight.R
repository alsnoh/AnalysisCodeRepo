...
"pltName" <- paste("plt", scenario, sep = "")
...
pltName <- ggplot(data = DFLength, aes(x = 1:length(DFLength$lengths), y = lengths)) +
  geom_line() +
  theme_minimal() +
...
#ggsave(paste0("figures/yearlyLength/jd ", DFWeight$jd[1], "-", DFWeight$jd[nrow(DFWeight)], " ", scenario," ", DFWeight$year[1], " 5 years test.png"), plot = plt2, width = 18, height = 20, unit = "cm")
